...
export default function AnnouncementsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Duyuruları getir (Admin tümünü görebilir)
            const { data: annData } = await supabase
                .from("announcements")
                .select("*")
                .eq("tenant_id", null) // Sadece Admin'in oluşturdukları
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (targetType === "specific" && selectedCompanyIds.length === 0) {
            alert("Lütfen en az bir şirket seçin");
            return;
        }
        ...
        try {
                ...
                .insert([
                    {
                        ...
                        created_by: user!.id,
                        tenant_id: null, // Admin oluşturuyor
            ...
            if (targetType === "specific" && newAnn) {
                const targetInserts = selectedCompanyIds.map(id => ({
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold tracking-tight">Sistem Duyuruları</h1>
            </div>
