...
export default function ProfileCompletion({ onComplete }: ProfileCompletionProps) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900 bg-opacity-95 flex items-center justify-center p-4 z-[9999]">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 max-w-md w-full animate-fade-in relative overflow-hidden border border-transparent dark:border-slate-800">
                {/* Dekoratif Çizgi */}
                <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-red-500 to-red-600"></div>

                <div className="text-center mb-6 mt-4">
                    <div className="mx-auto w-12 h-12 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mb-3">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                        </svg>
                    </div>
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Profil Bilgileri Eksik</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Adınız *</label>
                            <input
                                type="text"
                                value={firstName}
                                onChange={(e) => setFirstName(e.target.value)}
                                className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                placeholder="Adınız"
                                required
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Soyadınız *</label>
                            <input
                                type="text"
                                value={lastName}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">TC Kimlik Numaranız *</label>
                        <input
                            type="text"
                            maxLength={11}
                            value={tcNo}
                            ...
                            required
                        />
                        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Sistem modüllerindeki atama ve doğrulama işlemleri için gereklidir.</p>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Telefon Numaranız *</label>
                        <input
                            type="tel"
                            value={phoneNumber}
                            ...
                            required
                        />
                        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Acil durumlar ve iletişim işlemleri için gereklidir.</p>
                    </div>
